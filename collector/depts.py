...
# 부서가 아니라 사람 자리인 것들. 순서가 중요하다 — 부교육감을 교육감보다 먼저 본다.
#
# 종류는 **본청**이다. 예전에는 `기관장` 이라는 종류를 따로 뒀는데, 화면에서 부서를
# 본청·직속기관·교육지원청으로 묶고 보니 부교육감 혼자 묶음 하나를 차지하고
# 본청과 직속기관 사이에 끼어 앉았다. 보는 사람에게 부교육감은 본청이다.
...
def affiliation(title: str | None) -> tuple[str | None, str]:
    ...
    소속 종류는 `본청` / `직속기관` / `교육지원청` / `의회` / `기타`.
    교육감·부교육감도 `본청` 으로 본다. 알 수 없으면 (None, '기타').
    """
    if not title:
        return (None, "기타")
